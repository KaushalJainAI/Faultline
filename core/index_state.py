...
_done_event.set()   # starts in "done" state — no indexing in progress
_db_path: str = ""
...
def start_background_index(target_dir: str, db_path: str) -> None:
    ...
    def _run() -> None:
        ...
        try:
            ...
            logger.info("index_state: background indexing complete → %s", db_path)
    ...
    _thread.start()
    logger.info(
        "index_state: background indexing started for %s → %s", target_dir, db_path
    )
