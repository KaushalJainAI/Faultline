...
def format_model_list(models: Optional[List[ModelEntry]] = None) -> str:
    ...
    lines = [
        "  # │ Provider    │ Model                          │ Value                              │ Context │ Free",
        "  ──┼─────────────┼────────────────────────────────┼────────────────────────────────────┼─────────┼─────",
    ]
    for i, m in enumerate(models, 1):
        ...
        lines.append(
            f"  {i:>2} │ {m.provider:<11} │ {m.name:<30} │ {m.value:<34} │ {ctx:<7} │  {free}"
        )
    return "\n".join(lines)


# ---------------------------------------------------------------------------
# Dynamic Overrides — loads from model_registry_overrides.json if present
# ---------------------------------------------------------------------------
