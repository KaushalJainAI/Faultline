...
Storage Layout:
    ~/.faultline/
        sessions/
            <project-slug>/                  â† one directory per target project
                <session-id>.jsonl           â† full conversation log (append-only)
                sessions-index.json          â† metadata index for all sessions
        memory/
            <project-slug>/
                MEMORY.md                    â† persistent cross-session context
        history.jsonl                        â† global session index across all projects
...
def _project_slug(target_dir: str) -> str:
    ...
    e.g., C:\\Users\\kj\\Desktop\\AIAAS\\Backend â†’ C-Users-kj-Desktop-AIAAS-Backend
    """
...
# ---------------------------------------------------------------------------
# SessionStore â€” the main interface
# ---------------------------------------------------------------------------
