...
Resolution order per role:
  1. token present in file                  â†’ use directly
  2. username + password + login_url set    â†’ caller must do login flow
  3. basic auth                             â†’ encode username:password directly
...
File layout:
    ...
    token    = ""   # leave blank â†’ auto-login via login_url or HITL
...
class CredentialStore:
    ...
    def load(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    "tomllib/tomli not available â€” cannot read credentials.toml. "
    ...
    def needs_login(self, role: str = "default") -> bool:
        ...
        login_url is configured â€” i.e. an auto-login flow is needed.
    ...
    def get_auth_header(self, role: str = "default", token_override: str = "") -> Optional[Dict[str, str]]:
        ...
        Supports:
          ...
          basic    â†’  {"Authorization": "Basic <b64(user:pass)>"}  (no token needed)
          cookie   â†’  {"Cookie": "session=<token>"}
          none     â†’  {}

        Returns None when the required credential is genuinely missing
        (e.g. bearer with no token and no login_url â€” needs HITL).
        """
        cred = self.get(role)
        if cred is None:
            return None
...
# â”€â”€ Module-level store â€” populated by faultline.py before the agent runs â”€â”€â”€â”€
...
def init_store(target_dir: str, credentials_path: Optional[str] = None) -> "CredentialStore":
    ...
    credentials_path â€” explicit path to a credentials.toml file (e.g.
